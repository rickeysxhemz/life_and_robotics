...
health robustness speed_variation
per_vis_rad per_vis_ang
food_around_me closest_food
have_venom
vis_rand
]
food-own [amount]
...
globals [rad
daytime starting_color current_color
...
to setup
  ...
  ;after rad addition globals added then setup this function
  set timer_reset 1000
  set daytime true
  set starting_color 85
  set current_color starting_color
  set color_range 5
  set color_adjust ( color_range / ( timer_reset + 10 ))


  create-peoples number_of_people[
    setxy random-xcor random-ycor
    set color grey
    set size 10
    set shape "person"
  ]
  create-butterflies number_of_butterflies[
    setxy random-xcor random-ycor
    set color blue
    set size 10
    set shape "butterfly"

    ;making butteflies unique
    set health 50 + random 50
    adjust_vision_cone
    set robustness random 10
    set speed_variation random 10
    set heading 0
    pendown
...
to draw_building
  ...
  ask patches with [ pxcor >= -30 and pxcor <= 30 and pycor >= -30 and pycor <= 30][
    set pcolor brown
    set solid true
  ]
end
...
to pickup_venom
  let pickup [false]
  ask venom in-radius rad [
    set pickup true
    die
  ]
  if pickup = true [
    set have_venom have_venom + 1
  ]
end
...
to-report people_function
  let seen [false]
  let hit [false]
  let person_hit 0

  ask peoples in-cone vis_rad vis_ang[
      set color green
      set seen true
    ]
...
to-report food_function [senstivity]
  set food_around_me other ( food in-radius senstivity )
...
"" ""
PENS
"Food" 1.0 0 -7500403 true "" "plot count food"
"Butterflies" 1.0 0 -2674135 true "" "plot count butterflies"
"People" 1.0 0 -955883 true "" "plot count peoples"
"Vpods" 1.0 0 -6459832 true "" "plot count venom"
